...
abstract contract UserOp is Test, SignerHelper {
    ...
    function _createUserOperation(
        ...
        if (isBatch) {
            callData = abi.encodeCall(IKintoWallet.executeBatch, (opParams.targets, opParams.values, opParams.bytesOps));
        } else {
        ...
        op.signature = _signUserOp(op, IKintoWallet(payable(from)).entryPoint(), chainID, privateKeyOwners);
        return op;
    }
